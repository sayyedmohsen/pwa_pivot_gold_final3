const apiKey = "8eca64515f2a4e58a6ee1152d5fc384b";
const symbol = "XAU/USD";

// تابع برای محاسبه تاریخ امروز به فرمت UTC
function getTodayUTCDate() {
  const now = new Date();
  return new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate())).toISOString().split("T")[0];
}

const today = getTodayUTCDate();

// محاسبه تاریخ روز گذشته
const yesterday = new Date();
yesterday.setDate(yesterday.getDate() - 1);
const startDate = yesterday.toISOString().split("T")[0] + " 00:00:00";
const endDate = yesterday.toISOString().split("T")[0] + " 23:59:00";

// درخواست به API برای دریافت اطلاعات روز گذشته
fetch(`https://api.twelvedata.com/time_series?symbol=${symbol}&interval=1day&apikey=${apiKey}&start_date=${startDate}&end_date=${endDate}&timezone=UTC`)
  .then(res => res.json())
  .then(data => {
    const values = data.values[0];
    const high = parseFloat(values.high);
    const low = parseFloat(values.low);
    const close = parseFloat(values.close);
    const open = parseFloat(values.open);

    // محاسبه پیوت پوینت به روش اصلی
    const pivot = (high + low + close) / 3;
    const R1 = (2 * pivot) - low;
    const R2 = pivot + (high - low);
    const R3 = high + 2 * (pivot - low);
    const S1 = (2 * pivot) - high;
    const S2 = pivot - (high - low);
    const S3 = low - 2 * (high - pivot);

    // ساخت جدول برای نمایش پیوت پوینت
    const resultTable = `
      <table>
        <tr><th>Date (UTC)</th><th>Pivot</th><th>R1</th><th>R2</th><th>R3</th><th>S1</th><th>S2</th><th>S3</th></tr>
        <tr>
          <td>${today}</td>
          <td>${pivot.toFixed(2)}</td>
          <td>${R1.toFixed(2)}</td>
          <td>${R2.toFixed(2)}</td>
          <td>${R3.toFixed(2)}</td>
          <td>${S1.toFixed(2)}</td>
          <td>${S2.toFixed(2)}</td>
          <td>${S3.toFixed(2)}</td>
        </tr>
      </table>
    `;

    document.getElementById("modified-results").innerHTML = resultTable;

    // ذخیره‌سازی تاریخ و پیوت پوینت‌های محاسبه‌شده
    const pivotData = {
      date: today,
      pivot: pivot.toFixed(2),
      R1: R1.toFixed(2),
      R2: R2.toFixed(2),
      R3: R3.toFixed(2),
      S1: S1.toFixed(2),
      S2: S2.toFixed(2),
      S3: S3.toFixed(2),
    };
    
    // ذخیره در localStorage برای استفاده در روزهای بعد
    localStorage.setItem("pivotData", JSON.stringify(pivotData));
    localStorage.setItem("lastUpdatedDate", today);
  })
  .catch(err => {
    console.error("API error:", err);
    document.getElementById("modified-results").innerHTML = `<p>خطا در دریافت داده‌ها</p>`;
  });
